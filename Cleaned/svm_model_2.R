...
for(lag_steps in start_lag_step:end_lag_step){
  ...
  if(!need_to_add_close_lagged){
    #first we lag then we split
    # train_df_wo_date <- lag_dataset(subset(train_df_wo_date,select=-Close),
    #                                 steps=lag_steps)
    # train_df_wo_date$Close <- train_df$Close[(lag_steps+1):nrow(train_df)]
    # test_df_wo_date <- lag_dataset(subset(test_df_wo_date,select=-Close),
    #                                steps=lag_steps)
    # test_df_wo_date$Close <- test_df$Close[(lag_steps+1):nrow(test_df)]
    
    df_wo_date<-lag_dataset(subset(df_wo_date,select=-Close),steps = lag_steps)
  }else{
  ...
  df_long_best <- df_lagged[(580-lag_steps):(827-lag_steps),]
  ...
  mape_metrics[mape_metrics$Step == lag_steps,"MAPE"]<-MAPE(df_long_best$predicted,df_long_best$Close)
