...
for(i in 1:lag_steps){
  ...
  data2_lagged <- data2_lagged %>% 
    left_join((subset(Bitcoin_lagged,select=c("ds",regressor_name))),by=c('ds'='ds'))
}
...
#### TODO add close lagged regressors
for(i in 1:lag_steps){
  regressor_name <- paste("Close_t_",i,sep="")
  print("adding regressor")
  print(regressor_name)
...
for(i in 1:lag_steps){
  regressor_name <- paste("Close_t_",i,sep="")
  future_p[regressor_name]<- Bitcoin_lagged[regressor_name]
}
