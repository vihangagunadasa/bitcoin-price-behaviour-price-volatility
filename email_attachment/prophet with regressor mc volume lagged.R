...
colnames(Bitcoin)[length(colnames(Bitcoin))]<-"MC"
...
Bitcoin <- mutate(Bitcoin,ds=Date,y=Close/close_divisor,
                  Volume_divided=Volume/volume_divisor, MC_divided=MC/mc_divisor) %>%
  subset(select=-c(Volume,MC))
...
Bitcoin_lagged <- lag_dataset(subset(Bitcoin,select=-Date),steps=lag_steps)
Bitcoin_lagged$ds <- Bitcoin$Date[(lag_steps+1):nrow(Bitcoin)]
...
#### TODO add_regressors market cap and volume lagged
for(i in 1:lag_steps){
  regressor_name <- paste("MC_divided_t_",i,sep="")
  print("adding regressor")
  print(regressor_name)
  p=add_regressor(p,regressor_name,standardize = F)
}
...
MC_prophet <- data2 %>% 
  mutate(y=MC_divided) %>%
  subset(select=c(ds,MC_divided,y)) 
MC_prophet_lagged <- lag_dataset(subset(MC_prophet,select=-ds),steps = lag_steps) 
MC_prophet_lagged$ds <- MC_prophet$ds[(lag_steps+1):nrow(MC_prophet)]
...
for(i in 1:lag_steps){
  ...
  print("adding regressor")
  print(regressor_name)
  MC=add_regressor(MC,regressor_name,standardize = F)
}
MC=fit.prophet(MC,MC_prophet_lagged)
future_MC=make_future_dataframe(MC,periods = 248,freq = "day")
for(i in 1:lag_steps){
  regressor_name <- paste("MC_divided_t_",i,sep="")
  future_MC[regressor_name] <- Bitcoin_lagged[regressor_name]
}
...
Volume_prophet <- data2 %>% 
  ...
  subset(select=c(ds,Volume_divided,y))
Volume_lagged <- lag_dataset(subset(Volume_prophet,select=-ds),steps=lag_steps)
Volume_lagged$ds <- Volume_prophet$ds[(lag_steps+1):nrow(Volume_prophet)]
#View(Volume_prophet)
...
for(i in 1:lag_steps){
  ...
  print("adding regressor")
  print(regressor_name)
  v=add_regressor(v,regressor_name,standardize = F)
}
v=fit.prophet(v,Volume_lagged)
