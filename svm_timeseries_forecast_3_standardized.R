...
df_long <- df
df_long$predicted <- predict(svmodel,subset(df_wo_date,select=-Close))
...
svmodel_best <- svm(Close ~ .,data=df_wo_date, type="eps-regression",kernel="radial",cost=svm_tune$best.parameter$cost, 
                    gamma=svm_tune$best.parameter$gamma)
df_long_best<-df
df_long_best$predicted <- predict(svmodel_best,subset(df_wo_date,select=-Close))
