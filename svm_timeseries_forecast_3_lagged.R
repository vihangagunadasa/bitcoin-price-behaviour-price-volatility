...
mape_metrics[mape_metrics$Step == step,"MAPE"]<-MAPE(df_long_best$predicted,df_long_best$Close)
series_df <- data.frame(
  Date=df_long_best$Date,
  Predicted=df_long_best$predicted
)
colnames(series_df)[2] <- paste("Predicted_step",step,sep="_")
...
print(mape_metrics)
write.csv(mape_metrics,"mape_metrics.csv",row.names=F)
